...
<script>
    $(function () {
       ...
       fit:true,
            onDestroy:function(){
                $("#dg").edatagrid('reload');//刷新当前datagrid
            },
            columns:[[
                {title:'cks',field:"cks",checkbox:true},
                {title:'id',field:'id',width:120,sortable:true},
                {title:'name',field:'name',width:120,editor:"text",},
                {title:'imgPath',field:'imgPath',width:120,sortable:true,editor:"text",},
                {title:'status',field:'status',width:120,sortable:true,editor:"text",},
                {title:'date',field:'date',width:120,sortable:true,editor:"datebox",},
                {title:'option',field:'option',width:120,sortable:true,editor:"text" ,
                    formatter:function(value,row,index){
                        return "<a href='javascript:;' class='options' onclick=\"openEditbannerDialog('"+row.id+"')\" data-options=\"iconCls:'icon-edit',plain:true\">更新信息</a>";
                    }
                }
            ]],
            view: detailview,
            detailFormatter: function(rowIndex, rowData){
                return '<table><tr>' +
                    '<td rowspan=2 style="border:0"><img src="${pageContext.request.contextPath}/' + rowData.imgPath + '" style="height:150px;"></td>' +
                    '<td style="border:0">' +
    ...
    function openEditbannerDialog(id){
        $("#editBannerDialog").dialog({
            href:'${pageContext.request.contextPath}/bannerjsp/edit.jsp?id='+id,
            buttons:[
                {
                    iconCls:'icon-save',
                    text:"修改",
...
<%--保存用户对话框--%>
<div id="saveUserDialog" data-options="href:'${pageContext.request.contextPath}/bannerjsp/save.jsp',draggable:false,iconCls:'icon-save',width:600,height:400,title:'保存用户信息'">
...
<%--更新用户对话框--%>
<div id="editBannerDialog" data-options="draggable:false,iconCls:'icon-edit',width:500,height:300,title:'更新用户信息'">
